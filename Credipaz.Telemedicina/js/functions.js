...
var _F = {
	...
	onDrawStatusDoctor: function () {
		...
		if (_API.telemedicina.atendiendo == 1) {
			$(".btnDoctorAtencion").attr("data-action", "descansar").removeClass("btn-info").addClass("btn-danger").html("Atendiendo");
		}
	},
			...
			_API.onShowModal("modalEditChargeCode", "", _html, "modal-xl").then(function (_ret) {
				_API.method("telemedicina/monitoreo", { "modo": 4, "id": _id, "idUser": _API.id_user_log })
					.then(function (data) {
						...
						if (_F._interfaceActiva == "supervision") {
	...
	onGrabarAtencion: function (_this) {
		...
		if (_idTypeClose != "") { _params["idTypeTaskClose"] = _idTypeClose; }
